...
class PlotResult(object):
    ...
    def plot_resultado(self,X, U):
            ...
            for i,lb in enumerate(self.label):        
                ...
                ax1.plot(k/60,y,"-k", label='Valor esperado')
    ...
    def plot_y(self,ysp, y,ylim):
        ...
        for i,lb in enumerate(self.label_y):        
            ...
            ax1.plot(k/60,ysp[i,:]*sc[i],":b", label='Set point')
            ax1.plot(k/60,y[i,:]*sc[i],"-k", label='Medição')
            if i==1:
                ax1.plot(k/60,hlim[1,:],":r", label='Bound')
                ax1.plot(k/60,hlim[0,:],":r")
            # ax1.plot(k, pred_test[:,i]*sc[i],":",color='blue',lw=2,label='Predição')
            ax1.set_ylabel(lb,  fontsize=Font)
